...
public class Values {
	 ...
	 * 변수에 직접 접근할 수 있다.
	 * 
	 * 이 프로젝트가 Run 될때 jVM에 의해서 자동으로 생성되고
	 * 사용할 준비가 된다.
	 * 
	 ...
	 * 
	 * 각 항목의 item 값에 따라 여러 기능을 선택해야하는 코드가 여기저기 나타난다면 코드를 작성하면서
	 * item값을(숫자) 일일이 기억해야만 한다.
	 * 이러한 상황은 item. 값을 잘 못 지정하여 논리적 오류가 발생할 소지가 있다.
	 * 
	 * 이때 item값들을 static변수로 미리 만들어두고 필요한 곳에 적용하면 
	 * 코드를 작성하는 도중에 문법적인 검사가 논리적 오류를 피할 수 있다.
	 */
